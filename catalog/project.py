...
def restaurantsJSON():
    ...
    for category in categories:
        ...
        data.append([category.serialize, [i.serialize for i in items]])
...
def showCatalog():
    ...
    latest_items = []
    for item in items:
        latest_items.append([item.name, session.query(Category)
                            .filter_by(id=item.category_id).one().name])
        if len(latest_items) >= 10:
            break

    if 'username' not in login_session:
        return render_template('publiccatalog.html', categories=categories,
                               items=items, latest_items=latest_items)
    else:
...
def showItems(category_name):
    ...
    if 'username' in login_session and creator.id == login_session['user_id']:
        return render_template('item2.html', items=items, category=category,
                               creator=creator, items_number=len(items))
...
def showItem(category_name, item_name):
    ...
    if 'username' not in login_session or creator.id != \
        ...
        return render_template('itemdesc.html', item=item, category=category,
                               creator=creator)
